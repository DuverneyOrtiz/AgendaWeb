<?php
  ...
  class ConectorBD
  {
    ...
    function datosUsuario($email){
      $sql="SELECT * FROM <usuario></usuario> WHERE correo='".$email."'";
      return $this->ejecutarQuery($sql);
    }

    function obtenerEventos($id){
      $sql="SELECT titulo AS title, CONCAT(F_Inicio,' ',H_Inicio) As start, F_Final As end, id_evento AS id FROM evento WHERE id='".$id."'";
      return $this->ejecutarQuery($sql);
    }

    function eliminarEvento($evento,$usuario){
      $sql="DELETE FROM evento WHERE id_evento='".$evento."' AND id='".$usuario."'";
      return $this->ejecutarQuery($sql);
    }
